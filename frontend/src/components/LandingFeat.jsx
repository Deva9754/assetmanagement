...
//     // Simulated role logic (replace with real DB/Firestore lookup in production)
//     if (activeTab === 'admin') {
...
const LandingFeat = () => {
  ...
  const handleForgotPassword = () => {
    if (!email) {
      showSnackbar('Please enter your email to reset password.');
      return;
    }
    ...
    sendPasswordResetEmail(auth, email)
      ...
      .catch((error) => {
        console.error('Error sending reset email:', error);
        showSnackbar(`Failed to send reset email: ${error.message}`, 'error');
      });
  };

  return (
    <>
      {/* ... your features section remains unchanged ... */}

      <div id="login-section" className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900">Access Your Account</h2>
            <p className="mt-4 text-xl text-gray-600">Secure login for administrators and users</p>
